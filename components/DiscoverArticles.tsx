...
function DiscoverArticles({
  ...
  for (let i = 0; i < articleBlocks.length; i++) {
    if (i >= articleChunkBlocks.length) {
      blocks.push(
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 col-span-1 lg:col-span-4 row-span-2">
          {articleBlocks[i]}
        </div>
      );
    } else {
      if (lastInserted === 'left') {
        blocks.push(
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 col-span-1 lg:col-span-3 row-span-2">
            {articleBlocks[i]}
          </div>
        );
